                ...
                my_matrix_t *targets, my_params_t *hparams)
{
    ...
    double grad_bias;
    uint16_t i = 0;
    for ( ; i < hparams->iterations; i++) {
        if (P->funcs.error_func(P, inputs, targets) < hparams->threshold) break;
        P->funcs.grad_theta_func(P, inputs, targets, &grad_theta);

        my_matrix_multiplybyscalar(&grad_theta, -1 * \
                                hparams->alpha, &grad_theta_alpha);
